...
/**
 * REST controller for managing {@link com.concesionaria.app.domain.Coche}.
 */
@RestController
@RequestMapping("/api")
public class CocheResource {
     ...
     *         body the new coche, or with status {@code 400 (Bad Request)} if the
     *         coche has already an ID.
     ...
     *         or with status {@code 500 (Internal Server Error)} if the coche
     *         couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    ...
    public ResponseEntity<List<Coche>> getAllCochesByColor(@PathVariable String color) {
        log.debug("REST request to get a page of Coches");
        List<Coche> coches = cocheService.findByColor(color);
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the coche, or with status {@code 404 (Not Found)}.
     */
